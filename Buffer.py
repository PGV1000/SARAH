...
src_shape_path = glob.glob('.\Buffer\*.shp')[0]
print(src_shape_path)
print('Creating the output file name...')
output_shape_path = '.\Buffer\Buffer_' + os.path.basename(src_shape_path)
...
bufferedGeom = geom.Buffer(-0.01,1)
...
name = glob.glob(".\Polygonize\*tif")[0] # значение атрибута
...
# inputFeature = inputLayer.GetFeature(0)
# inputGeom = inputFeature.GetGeometryRef()
# bufferGeom = inputGeom.Buffer(-0.01)
...
# output_ds = driver.CreateDataSource(output_layername)
